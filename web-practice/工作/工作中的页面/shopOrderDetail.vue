<template>
  <div>
    ...
    <div style="width: 100%;overflow-y: scroll;">
      ...
      <div>
        ...
        <div style="background:white;margin-top:10px;border-bottom:1px #f3f3f3 solid;font-size:14px;padding-top:5px" v-if="order.sendType === 'tc' || order.sendType === 'express'">
          ...
          <div style="background:white;padding:2px 15px 10px 15px;font-size:12px;border-top:1px solid rgba(183, 173, 173, 0.72);">
            ...
            <span style="display:block;line-height:30px;color:#666;">门店地址：<span style="color: black;">{{shopaddress}}</span></span>
            <span style="display:block;line-height:30px;color:#666;">门店电话：<span style="color: black;">{{shopphone}}</span></span>
        ...
        <div style="background:white;margin-top:10px;border-bottom:1px #f3f3f3 solid;font-size:14px;padding-top:5px" v-if="order.sendType === 'md'">
          <span style="color:#A9A9A9;margin-left:10px;font-weight:bold;color:#666;">商家信息</span>
          <div style="background:white;padding:2px 15px 10px 15px;font-size:12px;border-top:1px solid rgba(183, 173, 173, 0.72);">
            <span style="display:block;line-height:30px;color:#666;">配送方式：<span style="color: black;" v-if="order.sendType === 'md'">门店自提</span></span>
            <span style="display:block;line-height:30px;color:#666;">门店地址：<span style="color: black;">{{shopaddress}}</span></span>
            <span style="display:block;line-height:30px;color:#666;">门店电话：<span style="color: black;">{{shopphone}}</span></span>
          </div>
        </div>
      ...
      <div class="clear" style="text-align: center;/*padding-bottom: 60px;position: fixed*/;width: 100%">
        <x-button v-if="order.orderState===3 && order.evaluate==='false'" @click.native="bindToestimate()" style="background-color: orange;color: #FFFFFF;width: 95%;/*margin-bottom: 60px;*/">去评价</x-button>
      </div>
    </div>
...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      // 退款详情
      bindTodetails (item) {
        this.$router.push({path: '/orderfirst', query: {order: JSON.stringify(this.order), goods: JSON.stringify(this.goods)}})
      },
      // 二维码
      qrcode () {
      ...
      // 申请退款按钮
      bindToRefund (item) {
        this.$router.push({path: '/orderthird', query: {order: JSON.stringify(this.order), goods: JSON.stringify(this.goods)}})
      },
      // // 机票提交退款
      // doRefund () {
